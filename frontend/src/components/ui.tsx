      ...
      <div className="relative z-10 min-h-screen bg-[#111317]/82 backdrop-blur-[1px] flex flex-col justify-between selection:bg-[#B8323B]/30 selection:text-white">
        ...
        <header className="border-b border-white/[0.08] bg-transparent backdrop-blur-md sticky top-0 z-30">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-3.5">
            ...
            <nav className="flex items-center gap-3 sm:gap-4 text-xs font-sans">
              {!isLoginPage && (
                <>
                  {[
                    ['/', 'Home'],
                  ...
                  <Link
                    to="/compare"
                    className="bg-[#B8323B] hover:bg-[#CB3842] text-white px-3.5 py-1.5 text-xs font-sans font-medium no-underline transition-all tracking-wide rounded-lg shadow-sm flex items-center gap-1 cursor-pointer"
                  >
                    <span>Compare</span>
                    <span>→</span>
                  </Link>

                  <div className="h-3.5 w-px bg-white/15 mx-1 hidden sm:block" />
                </>
              )}
              ...
              {user ? (
                <div className="flex items-center gap-2.5">
                  <div className="flex items-center gap-1.5 border border-white/10 bg-[#1E2127] px-2.5 py-1 rounded-md text-[11px] font-sans">
                    <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 shrink-0" />
                    <span className="text-stone-200 font-medium truncate max-w-[110px]">{user.name}</span>
                  </div>
                  <button
                    type="button"
                    onClick={handleLogout}
                    className="text-stone-400 hover:text-[#FFA1A8] text-xs font-mono transition-colors cursor-pointer px-1 py-1"
                    title="Sign out of PactLens"
                  >
                    Logout
                  </button>
                </div>
                ...
                !isLoginPage && (
                  <Link
                    to="/login"
                    className="border border-white/15 bg-white/[0.04] hover:bg-white/[0.08] hover:text-white text-stone-300 px-3 py-1.5 text-xs font-mono rounded-lg transition-colors no-underline"
                  >
                    Sign In
                  </Link>
                )
